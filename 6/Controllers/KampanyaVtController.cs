...
{
    public class KampanyaVtController : Controller
    {
        public IActionResult Index(string Adsoyad, string Tcno, string Eposta, string Kampanyano)
        {
            ...
            if (Adsoyad != null | Tcno != null | Eposta != null | Kampanyano != null)
            {
                try
                {
                    ...
                    komut2.Parameters.AddWithValue("@Adsoyad", Adsoyad);//XSS (Cross-site scripting) için Razor engine çıktıyı encode eder. Zararlı kodları engeller.
                    komut2.Parameters.AddWithValue("@Tcno", Tcno);
                    komut2.Parameters.AddWithValue("@Eposta", Eposta);
                    komut2.Parameters.AddWithValue("@Kampanyano", Kampanyano);
